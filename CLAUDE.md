...
- **git の identity と署名鍵** (`modules/home/git.nix`): `user.name` / `user.email` / `user.signingkey` を宣言せず、`~/.config/git/local.conf` を include するだけ。未設定のマシンでコミットが落ちるのは意図した挙動 (意図しないアドレスで黙って打つより良い)。`signingkey` が per-machine なのは秘密鍵が Secure Enclave から出せず複製できないため — マシンごとに別の鍵になる。`ignores` の `**/.claude/settings.local.json` も外さない
- **署名と認証は Secure Enclave の鍵** (`modules/home/git.nix` の `ssh-sign` / `modules/home/ssh.nix`): `sc_auth create-ctk-identity -k p-256-ne -t none` で作った鍵を使う。`~/.ssh/id_secure_enclave` は鍵本体ではなく**ハンドル**で、鍵はチップから出ない。`gpg.ssh.program` のラッパーが `/usr/bin/ssh-keygen` を呼ぶのは意図的 — `SSH_SK_PROVIDER` に渡す `ssh-keychain.dylib` が Apple 製で、nixpkgs の ssh-keygen では動かない。`-t none` は使用のたびの生体認証を求めない設定で、外すとコミットのたびに止まる
- **ssh** (`modules/home/ssh.nix`): `enableDefaultConfig = false;` は意図的 — HM の旧既定値は将来削除予定で、残すと switch のたびに warning が出る。`includes` の `~/.ssh/config.local` はマシン固有設定の逃がし口で、存在しなくてよい (ssh は無い Include を黙って無視する)。ssh は**先勝ち**なので `Include` はファイル先頭でなければならない
